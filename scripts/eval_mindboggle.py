...
Note that ``msm`` is *not* FSL's MSM: T1Prep's ``desc-msmsulc`` sphere is
Spherical Demons onto the fsLR average standing in for MSMSulc (see
:mod:`t1prep.fslr`).  To compare against real MSM, register the same subjects
with FSL's ``msm`` and point ``--sphere-file`` at its output.
"""
...
def find_t1prep_file(subject_dir: Path, code: str, fshemi: str,
    ...
    for column, hemi, subdir in ((1, fshemi, "surf"),
        ...
        pattern = table.pattern(code, column)
        ...
        glob = (pattern
                ...
                .replace("..", "."))
        candidates += sorted((subject_dir / subdir).glob(glob))
...
def cmd_project(args: argparse.Namespace) -> int:
    ...
    out_dir = Path(args.work) / args.space
    out_dir.mkdir(parents=True, exist_ok=True)

    manifest_path = Path(args.work) / f"manifest_{args.space}.json"
    manifest = {"space": args.space, "sphere": sphere_code,
    ...
    for n, subject in enumerate(subjects, 1):
        ...
        for fshemi in ("lh", "rh"):
            try:
                ...
                per_hemi = {}
                break
            template_sphere = (tmpl_root / template_dir
                               / template_pat.format(fshemi=fshemi))
